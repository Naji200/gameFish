...
class Boid {
  ...
  // Méthode qui permet de calculer la force de répulsion pour éviter un obstacle
  avoidObstacle(obstacle) {
    // Calculate future position
    let futurePos = this.pos.copy();
    futurePos.add(p5.Vector.mult(this.vel, 20));

    // Calculate distance to obstacle
    let distToObstacle = p5.Vector.dist(futurePos, obstacle.pos);
    
    if (distToObstacle < obstacle.r + this.r) {
      // Calculate steering force to avoid obstacle
      let force = p5.Vector.sub(futurePos, obstacle.pos);
      force.normalize();
